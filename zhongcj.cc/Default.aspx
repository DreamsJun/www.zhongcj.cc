<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Default.aspx.cs" Inherits="zhongcj.cc.Default" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="Body" runat="server">
<div class="banner">
     <div id="full-screen-slider">
	     <ul id="slides">
             <li style="background-image:url(/styles/images/banner_01.jpg); display:block;"><a href="">Banner1</a></li>
             <li style="background-image:url(/styles/images/banner_02.jpg);"><a href="">Banner1</a></li>
             <li style="background-image:url(/styles/images/banner_03.jpg);"><a href="">Banner1</a></li>
	     </ul>
     </div>
</div>
<div class="site_bg">
    <div class="h_product">
         <div class="hp_hot">
              <h2 class="hp_hot_title">热卖商品</h2>
              <ul class="hp_hot_items">
                  <li class="hot_item">
                      <a href="" target="_blank">
                         <img src="" alt="产品图片" />
                         <span>产品产品产品产品产品dsafsfsfsafsfsf2</span>
                      </a>
                      <span class="price">¥248.00</span>
                  </li>
                  <li class="hot_item">
                      <a href="" target="_blank">
                         <img src="" alt="产品图片" />
                         <span>产品1</span>
                      </a>
                      <span class="price">¥248.00</span>
                  </li>
                  <li class="hot_item">
                      <a href="" target="_blank">
                         <img src="" alt="产品图片" />
                         <span>产品1</span>
                      </a>
                      <span class="price">¥248.00</span>
                  </li>
                  <li class="hot_item">
                      <a href="" target="_blank">
                         <img src="" alt="产品图片" />
                         <span>产品1</span>
                      </a>
                      <span class="price">¥248.00</span>
                  </li>
                  <li class="hot_item">
                      <a href="" target="_blank">
                         <img src="" alt="产品图片" />
                         <span>产品1</span>
                      </a>
                      <span class="price">¥248.00</span>
                  </li>
                  <li class="hot_item">
                      <a href="" target="_blank" style=" border:0;">
                         <img src="" alt="产品图片" />
                         <span>产品1</span>
                      </a>
                      <span class="price">¥248.00</span>
                  </li>
              </ul>
         </div>
         
         <%--推荐，优惠，新品？--%>
         <div class="hp_rcm">
              <ul class="hp_rcm_items">
                  <li class="rcm_item" style="margin-left:0;">
                      <div class="rcm_item_title cit_bg">
                           <h5>精品推荐</h5>
                           <a href="" target="_blank" class="more">更多&raquo;</a>
                      </div>
                      <ul class="ri_product">
                          <li>
                              <a href="" target="_blank">
                                 <span>工厂直销浴巾</span>
                                 <img src="" alt="工厂直销浴巾" />
                              </a>
                          </li>
                          <li class="no_bordr">
                              <a href="" target="_blank">
                                 <span>100元以内让生活质量飙升的数码小物件</span>
                                 <img src="" alt="工厂直销浴巾" />
                              </a>
                          </li>
                          <li>
                              <a href="" target="_blank">
                                 <span>100元以内让生活质量飙升的数码小物件</span>
                                 <img src="" alt="工厂直销浴巾" />
                              </a>
                          </li>
                          <li class="no_bordr">
                              <a href="" target="_blank">
                                 <span>100元以内让生活质量飙升的数码小物件</span>
                                 <img src="" alt="工厂直销浴巾" />
                              </a>
                          </li>
                          <li style="border-bottom:0;">
                              <a href="" target="_blank">
                                 <span>工厂直销浴巾</span>
                                 <img src="" alt="工厂直销浴巾" />
                              </a>
                          </li>
                          <li class="no_bordr" style="border-bottom:0;">
                              <a href="" target="_blank">
                                 <span>工厂直销浴巾</span>
                                 <img src="" alt="工厂直销浴巾" />
                              </a>
                          </li>
                      </ul>
                  </li>
                  <li class="rcm_item">
                      <div class="rcm_item_title cit_bgo">
                           <h5>新品上市</h5>
                           <a href="" target="_blank" class="more">更多&raquo;</a>
                      </div>
                      <ul class="ri_product">
                          <li>
                              <a href="" target="_blank">
                                 <span>工厂直销浴巾</span>
                                 <img src="" alt="工厂直销浴巾" />
                              </a>
                          </li>
                          <li class="no_bordr">
                              <a href="" target="_blank">
                                 <span>100元以内让生活质量飙升的数码小物件</span>
                                 <img src="" alt="工厂直销浴巾" />
                              </a>
                          </li>
                          <li>
                              <a href="" target="_blank">
                                 <span>100元以内让生活质量飙升的数码小物件</span>
                                 <img src="" alt="工厂直销浴巾" />
                              </a>
                          </li>
                          <li class="no_bordr">
                              <a href="" target="_blank">
                                 <span>100元以内让生活质量飙升的数码小物件</span>
                                 <img src="" alt="工厂直销浴巾" />
                              </a>
                          </li>
                          <li style="border-bottom:0;">
                              <a href="" target="_blank">
                                 <span>工厂直销浴巾</span>
                                 <img src="" alt="工厂直销浴巾" />
                              </a>
                          </li>
                          <li class="no_bordr" style="border-bottom:0;">
                              <a href="" target="_blank">
                                 <span>工厂直销浴巾</span>
                                 <img src="" alt="工厂直销浴巾" />
                              </a>
                          </li>
                      </ul>
                  </li>
                  <li class="rcm_item">
                      <div class="rcm_item_title cit_bgt">
                           <h5>优惠促销</h5>
                           <a href="" target="_blank" class="more">更多&raquo;</a>
                      </div>
                      <ul class="ri_product">
                          <li>
                              <a href="" target="_blank">
                                 <span>工厂直销浴巾</span>
                                 <img src="" alt="工厂直销浴巾" />
                              </a>
                          </li>
                          <li class="no_bordr">
                              <a href="" target="_blank">
                                 <span>100元以内让生活质量飙升的数码小物件</span>
                                 <img src="" alt="工厂直销浴巾" />
                              </a>
                          </li>
                          <li>
                              <a href="" target="_blank">
                                 <span>100元以内让生活质量飙升的数码小物件</span>
                                 <img src="" alt="工厂直销浴巾" />
                              </a>
                          </li>
                          <li class="no_bordr">
                              <a href="" target="_blank">
                                 <span>100元以内让生活质量飙升的数码小物件</span>
                                 <img src="" alt="工厂直销浴巾" />
                              </a>
                          </li>
                          <li style="border-bottom:0;">
                              <a href="" target="_blank">
                                 <span>工厂直销浴巾</span>
                                 <img src="" alt="工厂直销浴巾" />
                              </a>
                          </li>
                          <li class="no_bordr" style="border-bottom:0;">
                              <a href="" target="_blank">
                                 <span>工厂直销浴巾</span>
                                 <img src="" alt="工厂直销浴巾" />
                              </a>
                          </li>
                      </ul>
                  </li>
              </ul>
         </div>
         <%-- End 推荐，优惠，新品？--%>


         <div class="hp_category">
              <div class="hpc_title"><h3>1F　钢化玻璃膜</h3><a href=""  target="_blank" class="more">更多&raquo;</a></div>
              <div class="hpc_body">
                   <div class="hpcd_rcm">
                        <a href="" title=""><img src="" alt="广告位" /></a>
                   </div>
                   <ul class="hpcd_product">
                        <li class="h_product_item">
                             <a class="" target="_blank" title=""><img src="" alt="工厂直销浴巾" /></a>
                             <a class="" target="_blank"></a>
                             <span class="price">¥2.00元</span>
                        </li>
                        <li class="h_product_item">
                             <a class="" target="_blank" title=""><img src="" alt="工厂直销浴巾" /></a>
                             <a class="" target="_blank"></a>
                             <span class="price">¥2.00元</span>
                        </li>
                        <li class="h_product_item">
                             <a class="" target="_blank" title=""><img src="" alt="工厂直销浴巾" /></a>
                             <a class="" target="_blank"></a>
                             <span class="price">¥2.00元</span>
                        </li>
                        <li class="h_product_item">
                             <a class="" target="_blank" title=""><img src="" alt="工厂直销浴巾" /></a>
                             <a class="" target="_blank"></a>
                             <span class="price">¥2.00元</span>
                        </li>
                        <li class="h_product_item">
                             <a class="" target="_blank" title=""><img src="" alt="工厂直销浴巾" /></a>
                             <a class="" target="_blank"></a>
                             <span class="price">¥2.00元</span>
                        </li>
                        <li class="h_product_item">
                             <a class="" target="_blank" title=""><img src="" alt="工厂直销浴巾" /></a>
                             <a class="" target="_blank"></a>
                             <span class="price">¥2.00元</span>
                        </li>
                        <li class="h_product_item">
                             <a class="" target="_blank" title=""><img src="" alt="工厂直销浴巾" /></a>
                             <a class="" target="_blank"></a>
                             <span class="price">¥2.00元</span>
                        </li>
                        <li class="h_product_item">
                             <a class="" target="_blank" title=""><img src="" alt="工厂直销浴巾" /></a>
                             <a class="" target="_blank"></a>
                             <span class="price">¥2.00元</span>
                        </li>
                   </ul>
              </div>
         </div>
    </div>
</div>
<script src="/javascript/bannerslide.js" type="text/javascript"></script>
</asp:Content>
